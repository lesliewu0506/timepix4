...
def VisualizeMultipleToTChargeSingle(filepath1, filepath2):
    ...
    for i, pixel in enumerate(range(50, 450, 50)):
        ...
        axs[j, k].scatter(df1["Charge"] / 1000, df1["meanTot"], color = "blue", label = "Mean ToT N10")
        ...
        axs[j + 2, k].scatter(df2["Charge"] / 1000, df2["meanTot"], color = "blue", label = "Mean ToT N116")
...
def STDHistogram(filepath1, filepath2, filepath3, filepath4):
    ...
    for filepath, color in zip([filepath1, filepath2, filepath3, filepath4], ['blue', 'orange', 'green', 'red']):
        filename = filepath.split("_")
        filename = filename[1].split(".")[0]
        df = CreateDataFrame(filepath)
...
if __name__ == "__main__":
    STDHistogram(f"{pre}fitData_N10.csv", f"{pre}fitData_N112.csv", f"{pre}fitData_N113.csv", f"{pre}fitData_N116.csv")
