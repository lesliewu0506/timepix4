...
def ConvertClusterData(filepath: str) -> None:
    ...
    ax.set_title("Charge [ke] per pixel with stage at (27.610/23.52/42.440)")
    plt.tight_layout()
    plt.savefig(f"Focus at ({center_col}, {center_row}) 3.png", dpi=300)
    plt.show()


def ZScanPlot(filepath):
    ...
    df_cltot["cltot"] = df_cltot["cltot"].astype(float) * CorrectionFactors[(228, 230)]
    mean = df_cltot["cltot"].mean()
    print(mean)

if __name__ == "__main__":
    # ConvertClusterData("N116-250425-111418.root")
    # ConvertClusterData("N116-250425-142003.root")
    # ConvertClusterData("N116-250425-143945.root")
