...
def process_folder(folder_path: str):
    ...
    df_mean.to_csv(
        f"Data/XFocus/ProcessedFocus {AttenuationVoltage} V/tot_{height_num}.csv",
        index=False,
    )
...
def ZScanPlot():
    df = pd.read_csv(f"Data/XFocus/ProcessedFocus {AttenuationVoltage} V/Results.csv")
    ...
    ax.scatter(
        ...
        color="red",
        s=100,
        label=f"Max ToT at x = {max_height} mm",
        zorder=3,
    )
    ax.set_xlabel("X Position Stage [mm]")
    ax.set_ylabel("ToT [25ns]")
    ax.set_title(
        f"Mean clToT and ToT vs X Position (Attenuation Voltage = {AttenuationVoltage} V; Pixel ({COL}, {ROW}))"
    )
    ...
    ax.grid(True)
    plt.tight_layout()
    plt.savefig(f"XScanPlot{AttenuationVoltage}.png", dpi=600)
    plt.show()


def ProcessFiles():
    data_root = f"Data/XFocus/Focus {AttenuationVoltage} V"
    results = []
    ...
    df_results.to_csv(
        f"Data/XFocus/ProcessedFocus {AttenuationVoltage} V/Results.csv", index=False
    )
